...
| **Pure AI wins (prompt → final)** | Spec layer becomes niche for "premium" or "complex" projects only. |
...
| **Kling AI (3.0 Latest)** | Yes, via Higgsfield platform (unlimited access with partnership). API for integrations. | Advanced controls: Multi-shot with camera movement (e.g., macro close-ups). Timing up to 15s continuous. Characters via reference videos for consistency. Native audio/lip-sync. Supports structured prompts but no full JSON pipeline. | Excellent multi-shot: Seamless sequences without stitching. Solves continuity issues like face morphing. Quality ceiling: Photorealistic for ads/UGC, but physics violations in complex actions. | Hybrid: Prompt → output with strong consistency; emerging as less black-box due to native multi-modal integration. |
...
Examples: Break into steps (research → outline → generate → edit); tools like Vellum AI for chaining.
...
---
## Round 2
### 🧭 Executive Summary: The State of Structured Control
...
### 🔬 Detailed Analysis by  Inquiry Framework
...
* **Long-form content**: This is the frontier. While Sora 2 can generate longer clips (up to 60 seconds), creating a coherent 3-minute video requires external scripting, shot-by-shot generation, and manual editing/stitching—a structured pipeline in itself.
...
## Key insight: blueprint.json shouldn't just validate — it should compile to multiple targets.
```
blueprint.json (Universal Spec)
       │
       ├──► Kling 3.0 Storyboard JSON (multi-shot API)
       ├──► Veo 3.1 JSON Prompt (community format)
       ├──► Sora 2 Structured Prompt (text with refs)
       ├──► ComfyUI Workflow Nodes (full injection)
       ├──► Traditional Pipeline (assets.md → manual tools)
       └──► Doxascope/My Tools (personal bridge)
```
This makes System 2 (NODE 3.5) a compiler, not just a router.

--

## Kling 2.6 (not 3.0) — Free tier limits:
...
## Kling API Analysis
### Canvas Features
"Storyboard" and "Multi-Angle Storyboard" — structured multi-shot input exists
...
"Generate next shot, use Start & End Frames" — shot chaining
Duration: 3s-10s granular control

### API - Text to Video — THE KEY
...
AI Multi-Shot — likely the 3.0 storyboard feature
Multi-Elements — character/object consistency
Lip-Sync — audio sync
Video Extension — shot chaining

### What This API Actually Does
Field	Reality
Input	Single frontal image of a character/element
Output	Multiple images (index 0-9) — different angles of same element
Purpose	Generate character reference sheets for consistency
Video?	No — this generates IMAGES, not video
...
Canvas "Storyboard" UI	Web only	❌ Not in API
"Multi-Elements" + chained calls	API	✅ Manual orchestration
"Video Extension"	API	✅ Shot chaining
The path to multi-shot video via Kling API:
1. Generate character elements (AI Multi-Shot → reference images)
